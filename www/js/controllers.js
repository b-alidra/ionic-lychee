...
.controller('AppCtrl', function($scope, $rootScope, $api, $state, $ionicHistory, $localStorage, $navHelper, $uploader, $ionicPlatform) {
    ...
    .controller('AlbumCtrl', function($scope, $rootScope, $api, $stateParams, $ionicPlatform, $ionicModal, $ionicSlideBoxDelegate, $cordovaImagePicker, $uploader) {
        /* Gallery stuff */
        $scope.album        = {};
        $scope.slides       = [];
            ...
            $scope.choosePhoto = function() {
                ...
                .then(function (results) {
                    ...
                    for (var i = 0; i < results.length; i ++) {
                        $rootScope.$apply(
                            $uploader.addUpload(
                                // Original file path, for upload
                                results[i],
                                // Copied file path, for preview
                                results[i],
                                // Album id
                                $scope.album.id,
                                // Album title
        ...
        $scope.$on('$destroy', function() {
            try{
                $scope.modal && $scope.modal.remove();
                $scope.uploadModal && $scope.uploadModal.remove();
            } catch(err) {
                console.log(err.message);
            }
        });
